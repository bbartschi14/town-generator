...
[CreateAssetMenu(menuName = "Building Generation/Wings/TWings")]
public class TWingsStrategy : WingsStrategy
{
    public override Wing[] GenerateWings(BuildingSettings settings)
    {
        if (settings.Size.x > 2 && settings.Size.y > 1)
        {
            ...
            RectInt secondBounds = new RectInt(settings.Size.x/2, 1, 1, settings.Size.y - 1);
